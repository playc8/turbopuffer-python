...
class VectorRow:
    ...
    @staticmethod
    def from_dict(source: dict[str, Any]) -> "VectorRow":
        return VectorRow(
            id=source.get("id"),
    ...
    def __post_init__(self):
        ...
        if self.vector is not None:
            if "numpy" in sys.modules and isinstance(
                ...
                if self.vector.ndim != 1: # type: ignore
                    raise ValueError(
                        f"VectorRow.vector must be a 1d array, got {self.vector.ndim} dimensions" # type: ignore
                    )
...
class VectorColumns:
    ...
    def __post_init__(self):
        if "numpy" in sys.modules and isinstance(
            ...
            if self.ids.ndim != 1:
                raise ValueError(
                    f"VectorColumns.ids must be a 1d array, got {self.ids.ndim} dimensions"
                )
        ...
        if "numpy" in sys.modules and isinstance(
            ...
            if self.vectors.ndim != 2:
                raise ValueError(
                    f"VectorColumns.ids must be a 2d array, got {self.vectors.ndim} dimensions"
                )
    ...
    def append(self, other) -> "VectorColumns":
        ...
        if isinstance(other, VectorRow):
            ...
            if other.distances is not None:
                if self.distances is None:
                    self.distances = [None] * old_len
                self.distances.extend(other.distances)
            elif self.distances is not None:
                self.distances.extend([None] * len(other.ids))
            new_len = len(self.ids)
            if other.attributes:
                if not self.attributes:
                    self.attributes = dict()
            ...
            if isinstance(other[0], VectorRow):
                ...
                if self.distances is not None or any(
                    row.dist is not None for row in other
                ):
                    if self.distances is None:
                        self.distances = [None] * old_len
    ...
    def from_rows(row_data: Union[VectorRow, Iterable[VectorRow]]) -> "VectorColumns":
        ...
        if isinstance(row_data, VectorRow):
            ...
            return VectorColumns(
                ...
                distances=distances if any(d is not None for d in distances) else None,
            )
            ...
            for i, row in enumerate(row_data):
                ...
                vectors.append(parsed_row.vector)
                distances.append(parsed_row.dist)
...
class VectorResult:
    ...
    @staticmethod
    def load_data(initial_data: Optional[DATA]) -> Optional[SET_DATA]:
        if initial_data:
    ...
    def __str__(self) -> str:
        if not self.next_cursor and self.offset == 0:
            ...
            return (
                "VectorResult("
                f"namespace='{self.namespace.name if self.namespace else 'None'}', "
    ...
    def __getitem__(self, index) -> VectorRow:
        if self.data is None:
            raise IndexError(f"Index {index} out of range for empty VectorResult")

        if isinstance(self.data, list):
            if index >= len(self.data) and self.next_cursor:
                it = iter(self)
                collected_data = list(self.data) if self.data else []
                for item in it:
                    collected_data.append(item)
                self.data = collected_data
                self.offset = 0
            ...
            raise TypeError(f"Cannot index into data of type {type(self.data)}")
    ...
    def __aiter__(self) -> "VectorResult":
        """
    ...
    def __next__(self) -> VectorRow:
        ...
        # This makes a synchronous API call
        ...
        self.offset += (
            0
            if self.data is None
            else (len(self.data) if isinstance(self.data, list) else len(self.data))
        )
        self.index = -1
    ...
    async def __anext__(self) -> VectorRow:
        ...
        # This makes an asynchronous API call
